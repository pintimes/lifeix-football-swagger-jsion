 ...
 * 
 * 1、指定文件
 * （1）指定文件夹列表，以及文件名列表（如["User.json","Wemedia.json"]，参数类型 List<String>）
 * （2）指定文件名列表（默认对inner和outer均生效）
 * （3）缺省该参数：默认对inner和outer文件夹中的所有json文件均生效
 * 
 * 2、指定属性
...
public class UpdateJsons {
    ...
    public static void doUpdate() throws Exception {
        List<String> dirs=Arrays.asList(CreateJsons.OUTER_DIR_NAME);
        List<UpdateConfig> list=Arrays.asList(
                new UpdateConfig(dirs,null,"$.paths..parameters", JSONObject.parseObject("{\""+CreateJsons.temp_$ref_replacer+"\": \"#/parameters/apiKeyHeader\"}"), 0),
                new UpdateConfig(dirs,null,"$.parameters.apiKeyHeader", JSONObject.parseObject("{"
        ...
        for (UpdateConfig updateConfig : list) {
            update(updateConfig.getDirs(),updateConfig.getFileNames(),updateConfig.getJsonpath(),updateConfig.getValue(),updateConfig.getOperateType());
        }
    ...
    private static List<SmbFile> getJsonFiles(List<String> dirs,List<String> fileNames) throws Exception{
        ...
        if (CollectionUtils.isEmpty(dirs)) {
            dirs=Arrays.asList(CreateJsons.INNER_DIR_NAME,CreateJsons.OUTER_DIR_NAME);
        }
    ...
    private static List<String> getAllJsonFileNames() throws Exception {
        List<String> dirPaths=Arrays.asList(CreateJsons.SMBDIR+CreateJsons.INNER_DIR_NAME+"/",CreateJsons.SMBDIR+CreateJsons.OUTER_DIR_NAME+"/");
    ...
    private static void updateJsonsAttrs(List<SmbFile> files, String jsonpath, Object value,int operateType) throws Exception {
        ...
        for (SmbFile smbFile : files) {
             ...
             * 更新json对象中的指定属性
             */
            updateJsonAttrs(jsonObject,jsonpath,value,operateType);
            /**
             * 回写json文件
             */
     ...
     * @throws Exception 
     */
    private static void updateJsonAttrs(JSONObject jsonObject, String jsonpath, Object value, int operateType)
            throws Exception {
        /**
         * 提取jsonpath内容
         */
